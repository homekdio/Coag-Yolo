...
from Improved.CoTAttention import CoTAttention
class Conv_CoT(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.CoT=CoTAttention(c2)

    def forward(self, x):
        return self.CoT((self.bn(self.conv(x))))

    def fuseforward(self, x):
        return self.CoT((self.conv(x)))
...
class CARAFE(nn.Module):
    def __init__(self, c, k_enc=3, k_up=5, c_mid=64, scale=2):
        ...
        """
        print("Alread loading CARAFE")
        print("k_enc=",k_enc,"k_up=",k_up)
