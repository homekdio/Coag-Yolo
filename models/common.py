...
class CoTAttention(nn.Module):
    """Contextual Transformer Attention 模块。
    融合静态上下文 (分组卷积提取的 Key) 与动态注意力 (自适应权重加权的 Value)，
    增强模型对全局长距离依赖的捕获能力。
    论文来源: Contextual Transformer Networks for Visual Recognition (2021)
    """
    ...
    def __init__(self, dim=512, kernel_size=3):
        ...
        # 动态注意力生成：瓶颈结构，输入为 Key 和 Query 的拼接
        factor = 4
...
class CARAFE(nn.Module):
    """CARAFE 内容感知上采样算子。
    利用特征图自身的语义信息预测每个像素的上采样核权重，
    替代传统最近邻 / 双线性插值上采样，最大程度保留语义细节。
    论文来源: CARAFE: Content-Aware ReAssembly of FEatures (ICCV 2019)
    """
    ...
    def __init__(self, c, k_enc=3, k_up=5, c_mid=64, scale=2):
        ...
        # PixelShuffle 将通道维重排到空间维
        self.pix_shf = nn.PixelShuffle(scale)

        # 最近邻上采样 + Unfold 提取局部邻域
    ...
    def forward(self, X):
        ...
        W = torch.softmax(W, dim=1)                     # b, k_up^2, h_, w_
